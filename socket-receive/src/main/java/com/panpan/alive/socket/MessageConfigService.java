...
public class MessageConfigService {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                while (canRun) {
                    try {
                        refresh();
                        int interval = NumberUtils.toInt((String) configParams.get("REFRESH_INTERVAL"), 60) * 1000;//
                        // 刷新间隔，单位：秒
    ...
    private void refresh() {
        ...
        if (hostAddress == null) {
            configParams.put("HOST_ADDRESS", "127.0.0.1");
        }
        ...
        if (publicKeyPath != null) {
            try {
                ...
                if (publicKeyPath.startsWith(classpathKey)) {
                    inputStream =
                            this.getClass().getClassLoader().getResourceAsStream(publicKeyPath.substring(classpathKey.length()));
                } else {
        ...
        if (publicKeyPath != null) {
            try {
                ...
                if (privateKeyPath.startsWith(classpathKey)) {
                    inputStream =
                            this.getClass().getClassLoader().getResourceAsStream(privateKeyPath.substring(classpathKey.length()));
                } else {
